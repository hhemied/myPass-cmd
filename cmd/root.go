...
// DBDir directort
var DBDir = filepath.Join(os.Getenv("HOME"), ".mypass-cmd")

// DBFile secret file
var DBFile = filepath.Join(DBDir, ".store")

// rootCmd represents the base command when called without any subcommands
var rootCmd = &cobra.Command{
	Use:   "myPass-cmd",
	Short: "myPass-cmd mange your secrets",
...
MyPass-cmd is a CLI Program for GNU/Linux, OSX and MS Windows.
...
func init() {
	...
	//rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.myPass-cmd.yaml)")
...
// DBExist to check if the DB file "~/.mypass-cmd/.store" is exist or not.
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".myPass-cmd" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".myPass-cmd")
	}
